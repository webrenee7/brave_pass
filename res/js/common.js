...
//前台登录验证 dubingnan
function loginProCheck(form)
{
	var username = form.username;
	var mobile = form.mobile;
	var cookietime = form.cookietime;
	if(username.value == ''){alert("请输入用户名");username.focus();return false;}
	if(mobile.value == ''){alert("请输入手机号");mobile.focus();return false;}
	var days = cookietime.value == 0 ? 1 : cookietime.value/86400;
	setcookie('username', username.value, days);
	return true;
}
...
function prompt(obj,title,description){
	var left=getLeft(obj)+obj.offsetWidth;
    var top=getTop(obj)+obj.offsetHeight;
    
    $.blockUI({
        message: '<h3>'+title+'</h3><p>描述：<br/>'+description+'</p><input type="button" value="关闭" onclick="$.unblockUI();"/>',
        css: {
            width: '200px',
            height: '150px',
            left: left,
...
$(function(){
	//调onchange事件
	//$("#area").trigger("onchange"); 
	$("#region").trigger("onchange"); 
	$("#province").trigger("onchange"); 	
})
function set_value(obj,id){
	if($(obj).find("option:selected").text()!="请选择"){
...
//军官证和身份证的切换
function identity_military(type)
{
	if(type == 0){
		$('#identity_card').show();
		$('#military_card').hide();
		$('.junguan').removeAttr('datatype');
		$('.shenfen').attr("datatype","idCard");
	}else{
		$('#identity_card').hide();
		$('#military_card').show();	
		$('.shenfen').removeAttr('datatype');
		$('.junguan').attr("datatype","limit");
	}
}
//dubingnan
